...
function getFullNames(runners) {
  ...
  runners.forEach(names => {
    return runnersNames.push(`${names.last_name}, ${names.first_name}`)
  })
  return runnersNames;
}
...
function firstNamesAllCaps(runners) {
  const nameCap = runners.map(names => {
...
function getRunnersByTShirtSize(runners, size) {
  const runFilter = runners.filter(sizes => {
    return sizes.shirt_size === size;
  });
  return runFilter;
}
...
function tallyUpDonations(runners) {
  const donTally = runners.reduce((acc, current) => {
